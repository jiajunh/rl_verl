...
def compute_egae_advantage_return(
    token_level_rewards: torch.Tensor,
    values: torch.Tensor,
    response_mask: torch.Tensor,
    gamma: torch.Tensor,
    ...
    with torch.no_grad():
        gae_nextvalues = 0
        gae_lastgaelam = 0
        gae_advantages_reversed = []

        egae_nextvalues = 0
        egae_lastgaelam = 0
        egae_advantages_reversed = []
        ...
        for t in reversed(range(gen_len)):
            # Original GAE
            gae_delta = token_level_rewards[:, t] + gamma * gae_nextvalues - values[:, t]
            gae_lastgaelam_ = gae_delta + gamma * lam * gae_lastgaelam
            # skip values and TD-error on observation tokens
            gae_nextvalues = values[:, t] * response_mask[:, t] + (1 - response_mask[:, t]) * gae_nextvalues
            gae_lastgaelam = gae_lastgaelam_ * response_mask[:, t] + (1 - response_mask[:, t]) * gae_lastgaelam
            gae_advantages_reversed.append(gae_lastgaelam)

            # EGAE
            response_length_idx = gen_len - t

            egae_delta = token_level_rewards[:, t] + gamma * egae_nextvalues - values[:, t]
            egae_lastgaelam_ = egae_delta + gamma * lam * egae_lastgaelam
            # skip values and TD-error on observation tokens
            if response_length_idx % window_length:
                egae_nextvalues = values[:, t] * response_mask[:, t] + (1 - response_mask[:, t]) * egae_nextvalues
            else:
                egae_nextvalues = values[:, t-1] * response_mask[:, t] + (1 - response_mask[:, t]) * egae_nextvalues

            egae_lastgaelam = egae_lastgaelam_ * response_mask[:, t] + (1 - response_mask[:, t]) * egae_lastgaelam
            egae_advantages_reversed.append(egae_lastgaelam)

        gae_advantages = torch.stack(gae_advantages_reversed[::-1], dim=1)
        egae_advantages = torch.stack(egae_advantages_reversed[::-1], dim=1)

        gae_returns = gae_advantages + values
        gae_advantages = verl_F.masked_whiten(gae_advantages, response_mask)

        egae_returns = egae_advantages + values
        egae_advantages = verl_F.masked_whiten(egae_advantages, response_mask)

    return gae_advantages, gae_returns, egae_advantages, egae_returns
