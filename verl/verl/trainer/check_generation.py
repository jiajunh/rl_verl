...
class TaskRunner:
    ...
    def run(self, config):
        ...
        print("-"*60)
        print("-"*60)
        print(f"Running only actor inference to test memory and dataset...")
        ...
        print(trainer.val_dataloader, len(trainer.val_dataloader))
        for i, test_data in enumerate(trainer.val_dataloader):
            print("-"*60)
            if i > 0: 
                break
            ...
            test_batch = test_batch.repeat(
                repeat_times=trainer.config.actor_rollout_ref.rollout.val_kwargs.n, interleave=True
            )
        ...
        # print(len(sample_inputs), len(sample_outputs), len(sample_gts))
        # output_file = trainer.config.actor_rollout_ref.model.path.split("/")[-1] + ".json"
        # print("save_file", output_file)
        # with open(output_file, "w") as f:
        #     for prompt, generation, gt in zip(sample_inputs, sample_outputs, sample_gts):
        #         json_line = {
        #             "question": prompt,
...
if __name__ == "__main__":
    ...
    #     "/n/netscratch/kdbrantley_lab/Lab/jiajunh/test_verl/models/rho_math_1b/rho-math-1b-v0.1",
    #     device_map="auto")
    # tokenizer = AutoTokenizer.from_pretrained("/n/netscratch/kdbrantley_lab/Lab/jiajunh/test_verl/models/rho_math_1b/rho-math-1b-v0.1")
